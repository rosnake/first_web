...
class TopicsAssessmentHandler(BaseHandler):
    ...
    用户议题显示处理
    """
    @handles_get_auth("/topics/assessment")
    def get(self):
        user_name = self.get_current_user()

        if user_name is not None:
            user_topic_tables = self.__get_assessment_issues_info()
    ...
    def __get_assessment_issues_info(self):
        ...
        for issues in issues_module:
            if issues.finish is False and issues.current is False:
                tmp = {
                    ...
                    "issues_evaluate_count": issues.issues_evaluate_count, "interested_count": issues.interested_count,
