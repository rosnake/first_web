...
class TopicsEvaluationHandler(BaseHandler):
    """
    用户议题显示处理
    """
    @handles_get_auth("/topics/evaluation")
    def get(self):
        user_name = self.get_current_user()

        if user_name is not None:
            user_topic_tables = self.__get_evaluation_issues_info()
            self.render("handlers/topic_evaluation.html",
    ...
    def __get_evaluation_issues_info(self):
        ...
        for issues in issues_module:
            if issues.finish is True and issues.issues_evaluate_finish is False:
                tmp = {
